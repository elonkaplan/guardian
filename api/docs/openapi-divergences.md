...
where the document and the design disagreed. **Row 5 is marked `DO NOT ADOPT`** — build
against the design for that one, not against the contract. Everything not listed here
matched and can be adopted as written.
...
| 5 | `GET /orders/{id}/case-file` → `steps`, buyer view | api-design §1.3: *"Execution steps are shown to buyers… The serialiser summarises reasoning text rather than passing it through raw"* | **always `[]`** for a buyer. The buyer's query does not select `runs.steps` at all | **`api-wrong`** | ⚠️ **NOT FIXED — `DO NOT ADOPT`.** See below |
...
## Row 5 in full — the one defect this report found
...
**What the code does.** `CaseFileService.getForBuyer` returns `steps: []`, unconditionally.
`findCaseFileForBuyer` does not select `runs.steps`, so the raw trace never enters the
...
**Why it was not fixed here.** The fix is small and the machinery already exists:
...
**What a consumer should do meanwhile.** Treat `steps` on a buyer's case file as always
empty. Do not build a buyer-facing trace view on it — it will render blank. The seller's
case file carries the populated trace. The operation's description in `openapi.yaml` carries
the same `DO NOT ADOPT` marker.
...
None. Row 5 is the only `api-wrong` row and it is deliberately left for its owner to decide;
every other row was resolved by correcting a design document or by recording a reason.

## Known wrong, not fixed

- **Row 5** — buyer case-file `steps` is always empty where the design says it carries a
  summarised trace. Marked `DO NOT ADOPT` in both this report and `openapi.yaml`.
